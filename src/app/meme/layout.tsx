...
const proxyImageUrl = (url: string): string => {
  if (!url) return url;
  if (url.includes('i-love-memes-backend.onrender.com/api/v1/files')) {
    return url.replace(
      'https://i-love-memes-backend.onrender.com/api/v1/files',
      '/api-proxy'
    );
  }
  if (url.includes('i-love-memes-files.s3.ap-southeast-2.amazonaws.com')) {
    return url.replace(
      'https://i-love-memes-files.s3.ap-southeast-2.amazonaws.com',
      '/s3-proxy'
    );
  }
  return url;
...
export default function MemeLayout({ children }: LayoutProps) {
  ...
  const loadBackgroundImage = useCallback(
    (imageUrl: string, currentZoom: number, currentRotation: number, config?: any) => {
      ...
      const bustUrl = `${proxyImageUrl(imageUrl)}?t=${Date.now()}`;

      FabricImage.fromURL(bustUrl, { crossOrigin: "anonymous" })
        .then((img) => {
  ...
  const addImageSticker = (imageUrl: string) => {
    ...
    FabricImage.fromURL(bustUrl, { crossOrigin: "anonymous" }).then((img) => {
