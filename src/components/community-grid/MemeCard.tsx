...
export function MemeCard({
  ...
  useEffect(() => {
    const t = setTimeout(() => setLoading(false), skeletonFor);
    return () => clearTimeout(t);
  }, [skeletonFor]);

  if (loading) return <MemeCardSkeleton />;
  ...
  return (
    <article className="border-1 border-[#D6C2FF] rounded-lg shadow-md p-2 flex flex-col 
      hover:shadow-lg transition-shadow group relative min-w-[160px] max-w-full w-full overflow-hidden pb-2">
      ...
      >
        ...
        >
          <div className="flex items-center gap-1">
            ...
            <div className="text-xs font-medium text-white px-2 py-0.5 rounded-full border border-white/50">
              {netScore}
            </div>
          </div>

          <div className="flex items-center gap-1">
            <button onClick={() => shareMeme(meme)} className="p-2 rounded-full hover:bg-white/30 cursor-pointer">
              <Share2 size={16} />
            </button>
            ...
            <button
              ...
              className={`p-2 rounded-full cursor-pointer transition-colors ${
                userHasFlagged ? "text-red-600 bg-red-200 hover:bg-red-300" : "hover:bg-white/30"
              }`}
            >
